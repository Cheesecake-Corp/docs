---
title: "Elektromagnetická indukce"
---

Stacionarní mg. pole: $\vec{B}$ se v daném místě mg. pole nemění s časem

Stacionární mg. pole je například v okolí vodiče s proudem

**Nestacionární mg. pole** -\> el. proud (nestacionární)

vzniká:

- pohybujícím se magnetem nebo elektromagnetem
- vodičem s časově proměnným el. proudem $I=I(t)$ - střídavý proud
- pohybem vodiče s elektrickým proudem stálým nebo časově proměnným

Nestacionární magnetické pole je příčinnou vzniku indukovaného elektrického pole a tento jev se nazývá **ELEKTROMAGNETICKÁ INDUKCE**. Na koncích vodiče vzniká indukované elektrické napětí $U_i$ a v uzavřeném obvodu teče indukovaný proud $I_i$. Například:

- magnet pohybující se uvnitř cívky
- na společném jádru jsou 2 cívky a v jedné cívce je proměnné elektrické pole.